  ...
  useEffect(() => {
    const fetchUpgrades = async () => {
      try {
        ...
        // Backend returns { upgrades: [...] }
        setUpgrades(data.upgrades || data || []);
      } catch (err) {
    ...
    <div className="p-4 flex flex-col h-full min-h-screen">
      <div className="sticky top-0 z-20 bg-[#0A0D14]/90 backdrop-blur-md pb-4 pt-2 -mx-4 px-4 border-b border-[#2D3748]/50 mb-4">
        ...
        <div className="mt-4 flex items-center justify-between glass-card p-3 rounded-xl border-[#B026FF]/30">
            ...
            <span className="font-bold text-[#00F0FF] neon-text-blue">{user?.coins?.toLocaleString() || 0} Neura</span>
        </div>
      </div>

      <div className="space-y-4 pb-28 flex-1 overflow-y-auto w-full">
        {upgrades.map((upgrade) => {
          const isPurple = upgrade.type === 'machine_multiplier';
          return (
            <div key={upgrade.upgradeId} className={`glass-card p-4 rounded-2xl relative overflow-hidden transition-all duration-300 border ${
              isPurple ? 'border-[#B026FF]/30 hover:border-[#B026FF]/60' : 'border-[#00F0FF]/30 hover:border-[#00F0FF]/60'
            }`}>
              <div className={`absolute -right-10 -top-10 w-24 h-24 rounded-full blur-[40px] opacity-10 pointer-events-none ${isPurple ? 'bg-[#B026FF]' : 'bg-[#00F0FF]'}`} />
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mt-2 pt-2 border-t border-[#2D3748]/50 gap-2">
                     <div className="flex items-center text-xs">
                       ...
                       <span className={`font-bold ${isPurple ? 'text-[#B026FF]' : 'text-[#00F0FF]'}`}>
                          {upgrade.effect}
                       </span>
                     </div>
                     
                     <button 
                       onClick={() => handleBuy(upgrade.upgradeId)}
                       disabled={purchasing === upgrade.upgradeId || (user && user.coins < upgrade.cost)}
                       ...
                       isPurple
                        ...
                        <span className="mr-1">{purchasing === upgrade.upgradeId ? 'Processing...' : 'Upgrade'}</span>
                        <span>{purchasing !== upgrade.upgradeId && upgrade.cost?.toLocaleString()}</span>
                     </button>
                  </div>
                </div>
              </div>
